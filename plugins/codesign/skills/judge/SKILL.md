...
1. **Preview** — `preview` every page that matters of the latest revision, in **one call**: `preview({ revision })` with `blockIds` omitted renders every page, so scoring the whole design costs no more than scoring page 1. Judging an old render is judging the wrong design. A render is invalidated by **content** changes, not by revision ids: if the latest revision is content-identical to one you already rendered this conversation (a no-op edit, a diagnostic/read-only pass, a rename), that render is still current — reuse it instead of re-rendering. **If the design is a localization, a resize, or a brand restyle, also `preview` the source revision** — you need both renders side by side for the source-parity checks above.
