...
| `scene/colorConversionMode`       | `Enum`   | `"Managed"`  | How this scene converts a CMYK or spot color into the color the engine displays. `Managed` converts through the document CMYK profile. `Legacy` inverts the ink channels and blends toward white for a tint, which is what scenes saved before managed conversion look like. This also selects how a CMYK or YCCK JPEG that carries no ICC profile of its own decodes: `Managed` converts its ink through the document CMYK profile, `Legacy` keeps the fixed inversion the image decoder applies., Possible values: `"Managed"`, `"Legacy"` |
